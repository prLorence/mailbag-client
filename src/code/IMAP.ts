...
export class Worker {
  ...
  public async listMessages(inMailbox: string): Promise<IMessage[]> {
    const response: AxiosResponse = await axios.get(`${config.serverAddress}/${inMailbox}`);
    return response.data;
  }
  ...
  public async deleteMessage(inID: string, inMailbox: string): Promise<void> {
    const response: AxiosResponse = await axios.delete(`${config.serverAddress}/messages/${inMailbox}/${inID}`)
    return response.data;
  }
}
